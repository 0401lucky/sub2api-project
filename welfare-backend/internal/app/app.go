...
func Build() (*gin.Engine, *config.Config, error) {
	...
	jwtService := service.NewJWTService(cfg.JWTSecret, cfg.JWTExpire)
	...
	authHandler := handler.NewAuthHandler(linuxdoService, authService, jwtService, cfg.FrontendCallbackURL, cfg.CookieSecure)
